...
export function useTournamentPage(
  ...
  const {
    ...
    [
      sessionContext,
      sessionFromHook.isAuthenticated,
      sessionFromHook.sessionData,
      sessionFromHook.updatePreferences,
    ],
  );
  ...
  const baseTournamentQuery = useMemo(
    () => graphql`
  ...
  const {tournament: baseTournamentData} = usePreloadedQuery(
    baseTournamentQuery,
    queryRef,
  );

  // Base tournament fragment (entries + breakdown + shell data)
  const baseTournamentFragment = useMemo(
    () => graphql`
  ...
  const commanderFragment = useMemo(
    () => graphql`
  ...
  const [baseTournament] = useRefetchableFragment(
    baseTournamentFragment,
    baseTournamentData,
  );
  const [commanderData, refetchCommander] = useRefetchableFragment(
    commanderFragment,
    baseTournamentData,
  );

  // Type cast and combine data
  const baseTournamentTyped = useMemo(
    () => baseTournament as any,
    [baseTournament],
  );
  const commanderDataTyped = useMemo(
    () => commanderData as any,
    [commanderData],
  );
  ...
  useEffect(() => {
    ...
    if (
      baseTournamentTyped &&
      (cacheExpired || !cachedBaseData?.entries?.length)
    ) {
      const newCacheData = {
  ...
  const handleCommanderSelect = useCallback(
    async (commanderName: string) => {
      ...
      await enhancedUpdatePreference(
  ...
  const originalQueryVariables = useMemo(
    () => queryRef.variables as any,
    [queryRef.variables],
  );
  ...
  const currentTabFromQuery = useMemo(() => {
    ...
    originalQueryVariables.showBreakdown,
    originalQueryVariables.showBreakdownCommander,
  ]);

  const commanderFromQuery = useMemo(
    () => originalQueryVariables.commander,
    [originalQueryVariables.commander],
  );
