...
interface CommandersGridProps {
  commanders: Array<{ node: { id: string } & commandersPage_TopCommandersCard$key }>;
  display: 'card' | 'table';
  secondaryStatistic: 'topCuts' | 'count';
}
...
// Pure UI Components
export const TopCommandersCard = React.memo<TopCommandersCardProps>(function TopCommandersCard({
  display = 'card',
  secondaryStatistic,
...
export function CommandersHeader({ display, onDisplayToggle, isAuthenticated }: CommandersHeaderProps) {
  return (
...
export function CommandersFilters({
  ...
  const currentSortByLabel = useMemo(
    () => currentPreferences?.sortBy === 'POPULARITY' ? 'Most Popular' : 'Top Performing',
  ...
  return (
    <div className="mb-8 flex flex-col items-start space-y-4 lg:flex-row lg:items-end lg:space-y-0">
      <div className="flex-1">
        <ColorSelection selected={currentPreferences.colorId} onChange={onColorChange} />
      </div>
...
export function CommandersGrid({ commanders, display, secondaryStatistic }: CommandersGridProps) {
