import React, { useMemo, useCallback, memo, MouseEvent, PropsWithChildren } from 'react';
...
interface TournamentPageShellProps {
  ...
  updatePreference: (key: keyof PreferencesMap['tournament'], value: any) => void;
...
export const BreakdownGroupCard = memo<BreakdownGroupCardProps>(function BreakdownGroupCard({
  onClickGroup,
  group: groupProp,
}) {
...
export const TournamentBanner = memo<TournamentBannerProps>(function TournamentBanner({
...
function useTournamentMeta(tournamentFromProps: tournamentPage_TournamentMeta$key) {
  const tournament = useFragment(
    graphql`
...
export const TournamentPageShell = memo<TournamentPageShellProps>(function TournamentPageShell({
  tab,
  commanderName,
  updatePreference,
  tournament: tournamentProp,
  ...
  return (
    <>
      <Navigation />
      <TournamentBanner tournament={tournament} isAuthenticated={isAuthenticated} />
...
export function TournamentContent({
  ...
  }, [contentType, entries, breakdownCards, commanderEntries, onCommanderSelect]);
