...
const TopCommandersCard = React.memo(({
  ...
  if (lazy && !shouldRender) {
    return (
      <div 
        ...
        className={cn(
          display === 'table' 
            ? "grid w-full grid-cols-[130px_1fr] items-center gap-x-2 overflow-x-hidden rounded-sm bg-[#312d5a]/25 p-4 text-white shadow-md lg:grid-cols-[130px_minmax(350px,1fr)_100px_100px_100px_100px] animate-pulse"
            : "bg-[#312d5a]/25 rounded-lg animate-pulse",
  ...
  if (display === 'table') {
    return (
      <div ref={ref} className="grid w-full grid-cols-[130px_1fr] items-center gap-x-2 overflow-x-hidden rounded-sm bg-[#312d5a]/50 p-4 text-white shadow-md lg:grid-cols-[130px_minmax(350px,1fr)_100px_100px_100px_100px]">
        <div>
...
const CommandersPageShell = React.memo(({
  ...
  const updateWithTransition = useCallback(<P extends keyof CommandersPreferences>(
    key: P, 
      ...
      <div className="mx-auto mt-8 w-full max-w-(--breakpoint-xl) px-8">
        <div className="flex w-full items-baseline gap-4">
          ...
          <div className="flex items-center gap-2">
            <button 
              className="cursor-pointer rounded bg-white/10 px-3 py-1 text-sm text-white hover:bg-white/20 transition-colors"
              onClick={toggleStats}
            >
              {(preferences?.statsDisplay || 'topCuts') === 'topCuts' ? 'Show Entries' : 'Show Performance'}
            </button>

            <button className="cursor-pointer hover:scale-105 transition-transform" onClick={toggleDisplay}>
              {display === 'card' ? (
...
> = ({queries}) => {
  ...
  const {preferences: commanderPrefs, updatePreference, isHydrated} = usePreferences(
    'commanders',
  ...
  const displayConfig = React.useMemo(() => {
    ...
    return {
      ...
      secondaryStatistic: (commanderPrefs?.statsDisplay || 'topCuts') as 'count' | 'topCuts',
      display
    };
