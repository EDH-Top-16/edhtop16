    ...
    for tournament in tqdm(db_1nf["metadata"].find()):
        ...
        for entry in db_1nf[tournament["TID"]].find():
            ...
            try:
                ...
                elif db_3nf["players"].count_documents({"name": entry["name"], "profile": entry["profile"]}) < 1:
                    player_id = db_3nf["players"].insert_one({"name": entry["name"], "profile": entry["profile"]}).inserted_id
                else:
                    player_id = db_3nf["players"].find_one({"name": entry["name"], "profile": entry["profile"]})["_id"]
            except Exception as e:
                print(f"Error while processing player -- tournament: {tournament['TID']}     entry: {entry['_id']}     error: {e}")

            # Add commander to collection if does not exist
            commander_id = None
            try:
                if "commander" not in entry.keys():
                    print(f"Warning: no commander for -- tournament: {tournament['TID']}     entry: {entry['_id']}")
                elif overwrite_data:
                ...
                elif db_3nf["commanders"].count_documents({"name": entry["commander"]}) < 1:
                    commander_id = db_3nf["commanders"].insert_one({"name": entry["commander"], "colorID": entry["colorID"]}).inserted_id
                else:
                    commander_id = db_3nf["commanders"].find_one({"name": entry["commander"]})["_id"]
            except Exception as e:
                print(f"Error while processing commander -- tournament: {tournament['TID']}     entry: {entry['_id']}     error: {e}")
            # TODO: Add/update entries collection with entries information
            try:
                if "decklist" not in entry:
                    entry["decklist"] = None
                if overwrite_data or db_3nf["entries"].count_documents({"playerID": player_id, "commanderID": commander_id}) < 1:
                ...
                print(f"Error while processing entry -- tournament: {tournament['TID']}     entry: {entry['_id']}     error: {e}")
