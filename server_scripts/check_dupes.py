...
if __name__ == '__main__':
    ...
    for TID in tqdm(existing_tournaments):
        ...
        for entry in db[TID].find():
            ...
            if (entry['name'] in names and entry['profile'] in names[entry['name']]) or entry['standing'] in standings:
                for standing, e in enumerate(tournament[0]['standings']):
                    if entry['name'] == e['name'] and entry['profile'] == e['profile']:
                        db[TID].delete_many({"name" : entry['name'], "profile": entry["profile"], "standing": {'$ne': standing + 1}})
                        db[TID].delete_many({"$or": [{"name": {"$ne": e['name']}}, {"profile": {"$ne": e['profile']}}, {"profile": {"$exists": False}}], "standing": standing + 1})
