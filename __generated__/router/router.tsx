 ...
 * @generated-checksum 44126b74fc9db620fc90076e3a3e10848a68678d9d3e1a7d69d3f4d1726cd3ff
    ...
    schema: z.object({
      ...
      sortBy: z.pipe(
        z.nullish(
          z.transform(
            (s: string) =>
              s as import('../queries/commanders_CommandersQuery.graphql').CommandersSortBy,
          ),
        ),
        z.transform((s) => (s == null ? undefined : s)),
      ),
      timePeriod: z.pipe(
        z.nullish(
          z.transform(
            (s: string) =>
              s as import('../queries/commanders_CommandersQuery.graphql').TimePeriod,
          ),
        ),
        z.transform((s) => (s == null ? undefined : s)),
      ),
    ...
    schema: z.object({
      ...
      sortBy: z.pipe(
        z.nullish(
          z.transform(
            (s: string) =>
              s as import('../queries/tournaments_TournamentsQuery.graphql').TournamentSortBy,
          ),
        ),
        z.transform((s) => (s == null ? undefined : s)),
      ),
      timePeriod: z.pipe(
        z.nullish(
          z.transform(
            (s: string) =>
              s as import('../queries/tournaments_TournamentsQuery.graphql').TimePeriod,
          ),
        ),
        z.transform((s) => (s == null ? undefined : s)),
      ),
...
function entrypoint_route(): EntryPoint<
  ModuleType<'route(/)'>,
...
function entrypoint_routestaples(): EntryPoint<
  ModuleType<'route(/staples)'>,
  { params: Record<string, unknown> }
> {
  return {
    root: JSResource.fromModuleId('route(/staples)'),
    getPreloadProps({ params }) {
      const { schema } = ROUTER_CONF['/staples'];
      const variables = schema.parse(params);
      return {
        queries: {},
        entryPoints: {
...
function entrypoint_routetournaments(): EntryPoint<
  ModuleType<'route(/tournaments)'>,
  { params: Record<string, unknown> }
> {
  return {
    root: JSResource.fromModuleId('route(/tournaments)'),
    getPreloadProps({ params }) {
      const { schema } = ROUTER_CONF['/tournaments'];
      const variables = schema.parse(params);
      return {
        queries: {},
        entryPoints: {
